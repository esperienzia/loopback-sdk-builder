  ...
  **/
  ...
  function buildServiceImports(model, loadAccessToken, isIo) {
    ...
    let imports = [
      ...
      { module: 'Subject, Observable', from: 'rxjs'},
      { module: 'map', from: 'rxjs/internal/operators' },
  ...
  function buildBaseServiceImports(isIo) {
    let imports = [
      { module: 'Injectable, Inject, Optional', from: '@angular/core'},
      { module: 'HttpClient, HttpRequest, HttpHeaders, HttpParams', from: '@angular/common/http'},
      { module: 'NgModule, ModuleWithProviders', from: '@angular/core'},
      ...
      { module: 'Observable, Subject, pipe', from: 'rxjs' },
      { module:  'map, catchError', from: 'rxjs/operators' },
    ];
  ...
  function paramIsQuery(param) {
    return (
      (
        ...
        (param.arg && !param.arg.match(/(^id$|fk|^file$|container)/) && !param.inBody) // But only if it is not id, fk, file or container
      )
      ||
      (
    ...
    if (Array.isArray(postData)) {
      ...
      if (postData.length > 0) {
        ...
        postData.forEach((property, i) => {
          if (property.http && property.http.source == 'form' || property.arg == 'password') {
  ...
  function buildUrlParams(model, methodName, urlParams) {
    ...
    urlParams = urlParams.filter(param => {
        // Filter out route params and function params
        if (paramIsRoute(param) || paramIsFunction(param) || paramIsContext(param) || paramIsBody(param) || param.arg == 'password') {
          return false
        }
